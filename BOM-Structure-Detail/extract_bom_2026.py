"""
2026-specific extraction: outputs output/bom_manhole_map.csv.

Prefers BOM by Structure (XML > PDF) over BOM Summary; falls back to Shop Drawing PDF.
Document type detection is driven by data/document_types.csv (no hardcoded keywords).
...
from parse_bom_pdf import (
    ...
    parse_ncf_docx, normalize_location,
)
...
def build_row(job_code, header, structure_name, year, item,
              source_file="", source_file_name="", ncf_meta=None):
...
def _process_by_structure_bom(job_code, file_item, bom, label, seen_structures, ncf_meta=None):
    rows = []
...
def process_by_structure_xml(job_code, file_item, xml_bytes, seen_structures, ncf_meta=None):
    ...
    return _process_by_structure_bom(job_code, file_item, bom,
                                     "BOM by Structure XML", seen_structures, ncf_meta)
...
def process_by_structure(job_code, file_item, pdf_bytes, seen_structures, ncf_meta=None):
    ...
    return _process_by_structure_bom(job_code, file_item, bom,
                                     "BOM by Structure PDF", seen_structures, ncf_meta)


def process_shop_drawing(job_code, file_item, pdf_bytes, seen_structures, ncf_meta=None):
    bom = parse_shop_drawing_pdf_safe(pdf_bytes)
    return _process_by_structure_bom(job_code, file_item, bom,
                                     "Shop Drawing PDF", seen_structures, ncf_meta)
...
def process_summary(job_code, file_item, pdf_bytes, seen_structures, ncf_meta=None):
    ...
    for item in bom["line_items"]:
        rows.append(build_row(job_code, bom["header"], structure, DRIVE_YEAR, item,
                              file_item["name"], "BOM Summary PDF", ncf_meta))
    return rows
...
def main():
    ...
    for folder in job_folders:
        ...
        try:
            ...
            # Pick highest-priority type group that has files
            ...
            for file_item in bom_files:
                ...
                try:
                    ...
                    if label == "BOM by Structure XML":
                        ...
                        rows = process_shop_drawing(job_code, file_item, file_bytes, seen_structures, ncf_meta)
                    else:
