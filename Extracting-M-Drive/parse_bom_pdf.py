...
# Matches any word-group category prefix at the start of a line (≤3 words), e.g. "Connectors 1 ..."
_ANY_CATEGORY_HEADER = re.compile(
    r"^([A-Za-z][A-Za-z&]*(?:\s+[A-Za-z&]+){0,2})\s+(?=\d)",
    re.IGNORECASE,
)
...
def _dedouble(s):
    """Collapse consecutive-duplicate character pairs: PPrreeccaasstt → Precast."""
