"""
Test extraction: outputs output/bom_manhole_map.csv matching Example Formatting.csv columns.

Prefers "BOM by Structure" PDFs (structure name comes from PDF content).
Falls back to "BOM Summary" PDFs (structure name parsed from filename).
...
from parse_bom_pdf import parse_bom_pdf_safe, parse_bom_by_structure_pdf_safe, parse_bom_by_structure_xml, parse_ncf_docx, normalize_location
...
JOBS_ROOT  = "root"
OUTPUT_DIR = "output"
OUTPUT     = os.path.join(OUTPUT_DIR, "bom_manhole_map.csv")
...
# Precast part number prefix -> Part Type
_PRECAST_TYPE_MAP = [
...
# Known diameters largest-first so greedy matching picks the right one
...
def is_bom_by_structure_xml(filename):
    n = filename.lower()
    return "bom by structure" in n and n.endswith(".xml")


def is_bom_by_structure(filename):
    n = filename.lower()
    return "bom by structure" in n and n.endswith(".pdf")


def is_bom_summary(filename):
    n = filename.lower()
    return "bom summary" in n and n.endswith(".pdf")


def is_ncf_ecf(filename):
    n = filename.lower()
...
def parse_structure_from_summary_filename(filename, pdf_job_name):
    """For BOM Summary files: extract structure tag from filename."""
    base = re.sub(r"-BOM Summary\.pdf$", "", filename, flags=re.IGNORECASE)
    rest = re.sub(r"^[A-Z]{3}\s+\d+\.\d+\.\d{2}\s+", "", base)
    clean_job = re.sub(r"^[A-Z]{3}\s+", "", pdf_job_name).strip()
...
def build_row(job_code, header, structure_name, year, item, ncf_meta=None):
    ...
    return {
        ...
        "Job Code":         job_code,
        "Project Name":     header["job_name"] or ncf.get("job_name", ""),
        "Structure Name":   structure_name,
        "Job Location":     job_location,
        "Location Source":  location_source,
        "Zip Code":         ncf.get("zipcode", ""),
        "Contractor":       header["contractor"] or ncf.get("customer", ""),
        "Agency":          "",
        "Engineer":        "",
        "Part Name":       pn_name,
...
def process_by_structure_xml(job_code, file_item, xml_bytes, seen_structures, ncf_meta=None):
    ...
    for structure in bom["structures"]:
        ...
        for item in structure["line_items"]:
            rows.append(build_row(job_code, bom["header"], name, DRIVE_YEAR, item, ncf_meta))
    return rows


def process_by_structure(job_code, file_item, pdf_bytes, seen_structures, ncf_meta=None):
...
def process_summary(job_code, file_item, pdf_bytes, seen_structures, ncf_meta=None):
    ...
    for item in bom["line_items"]:
        rows.append(build_row(job_code, bom["header"], structure, DRIVE_YEAR, item, ncf_meta))
    return rows
...
def main():
    ...
    for folder in job_folders:
        ...
        try:
            ...
            xml_boms     = sorted([f for f in all_files if is_bom_by_structure_xml(f["name"])],
                                  key=lambda f: _filename_sort_date(f["name"]), reverse=True)
            by_structure = sorted([f for f in all_files if is_bom_by_structure(f["name"])],
                                  key=lambda f: _filename_sort_date(f["name"]), reverse=True)
            summaries    = sorted([f for f in all_files if is_bom_summary(f["name"])],
                                  key=lambda f: _filename_sort_date(f["name"]), reverse=True)
            # NCF/ECF preferred; fall back to any .docx in folder if none found
            ncf_files = [f for f in all_files if is_ncf_ecf(f["name"])]
            if not ncf_files:
                ncf_files = [f for f in all_files if f["name"].lower().endswith(".docx")]

            if xml_boms:
                bom_files = xml_boms
            elif by_structure:
                bom_files = by_structure
            else:
                bom_files = summaries
            ...
            for ncf_file in ncf_files:
                try:
                    ncf_bytes = download_file(token, drive["id"], ncf_file["id"])
                    candidate = parse_ncf_docx(ncf_bytes)
                    if ncf_meta is None:
                        ncf_meta = candidate
                    if candidate.get("location"):
                        ncf_meta = candidate
                        print(f"[{job_code}] (ncf) {ncf_file['name']} -> {candidate['location']!r}")
                        break
                except Exception as exc:
                    print(f"[{job_code}] NCF parse failed ({ncf_file['name']}): {exc}")

            seen_structures = set()
            for file_item in bom_files:
                if is_bom_by_structure_xml(file_item["name"]):
                    mode = "xml"
                elif is_bom_by_structure(file_item["name"]):
                    mode = "by-structure"
                else:
                    mode = "summary"
                print(f"[{job_code}] ({mode}) {file_item['name']}")
                try:
                    file_bytes = download_file(token, drive["id"], file_item["id"])
                    if mode == "xml":
                        rows = process_by_structure_xml(job_code, file_item, file_bytes, seen_structures, ncf_meta)
                    elif mode == "by-structure":
                        rows = process_by_structure(job_code, file_item, file_bytes, seen_structures, ncf_meta)
                    else:
                        rows = process_summary(job_code, file_item, file_bytes, seen_structures, ncf_meta)
    ...
    if errors:
        print(f"\n{len(errors)} errors — see stderr above")
