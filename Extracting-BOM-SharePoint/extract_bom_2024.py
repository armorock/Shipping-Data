...
SITE_PATH  = "/sites/jobdata2024"
DRIVE_NAME = "Job Data 2024"
JOBS_ROOT  = "root"
OUTPUT_DIR = "output"
OUTPUT     = os.path.join(OUTPUT_DIR, "bom_manhole_map_2024.csv")
...
def _find_2024_site(token):
    """Search Graph for SharePoint sites containing '2024' and print candidates."""
    print("Searching for SharePoint sites containing '2024'...")
    try:
        results = graph_get(token, f"{GRAPH_ROOT}/sites", {"search": "2024"})
        sites = results.get("value", [])
        if not sites:
            print("  No sites found matching '2024'.")
        else:
