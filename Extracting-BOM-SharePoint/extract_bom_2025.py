...
JOBS_ROOT  = "root"
OUTPUT_DIR = "output"
OUTPUT     = os.path.join(OUTPUT_DIR, "bom_manhole_map_2025.csv")
...
def _find_2025_site(token):
    """Search Graph for SharePoint sites containing '2025' and print candidates."""
    print("Searching for SharePoint sites containing '2025'...")
    try:
        results = graph_get(token, f"{GRAPH_ROOT}/sites", {"search": "2025"})
        sites = results.get("value", [])
        if not sites:
            print("  No sites found matching '2025'.")
        else:
